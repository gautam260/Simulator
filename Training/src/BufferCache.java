...
public class BufferCache {
	void checkWorkingSet() {
		while(true) {
			...
			String lruChains= "select NXT_REPL,NXT_REPLAX,NXT_WRITE,NXT_WRITEAX from x$kcbwds where CNUM_SET>0";
			...
			try {
				...
				while (rs2.next()) {
					...
					System.out.println("");
					System.out.print("Total Buffers in LRU --> " + mainlruqueue.keySet().size() + " States ");
